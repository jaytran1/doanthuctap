...
class UserController extends Controller
{
    ...
    {
        $user = User::find($id);
        $khach = Khach::where('id',$id);
        if(count($user->khach)>0){
            $dem=count($user->khach);
            return redirect('admin/user/danhsach/')->with('thongbao','Admin này đang quản lí  '.$dem.' khách hàng, không thể xóa!');
        }
        $khachhang->delete();
        $user->delete();
